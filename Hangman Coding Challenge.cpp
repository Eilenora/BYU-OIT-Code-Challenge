...
bool checkInput(std::string board, std::string incorrectGuesses, std::string userGuessString)
{
...
bool checkIfCorrect(std::string board, std::string word, char userGuess)
{
...
int playGame()
{
    ...
    while (gameFinished == false) {
        ...
        while (!checkInput(board, incorrectGuesses, userGuessString))
        {
            std::cin >> userGuessString;
            userGuessString.at(0) = tolower(userGuessString.at(0));
        }
        ...
        if (checkIfCorrect(board, wordToGuess, userGuess))
        {
            numCorrectGuesses++;
